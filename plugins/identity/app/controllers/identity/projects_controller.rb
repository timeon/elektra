...
module Identity
  ...
  class ProjectsController < ::DashboardController
    before_filter :project_id_required, except: %i[index create
                                                   new user_projects]
    before_filter :identify_project_id, except: %i[index create new]
    ...
    def view
      @project = services.identity.find_project(@project_id, %i[subtree_as_ids
                                                                parents_as_ids])
    end
    ...
    def update
      enabled = params[:project_ng][:enabled]
      params[:project_ng][:enabled] = [true, 'true'].include?(enabled)
    ...
    def check_wizard_status
      unless @scoped_domain_name == 'ccadmin' &&
             @scoped_project_name == 'cloud_admin'
        service_names = %w[cost_control networking
                           resource_management].keep_if do |name|
          services.available?(name.to_sym)
        end
        project_profile = ProjectProfile
                          .find_or_create_by_project_id(@scoped_project_id)
    ...
    def update_resource_management_wizard_status
      if services.resource_management.has_project_quotas?
        @project_profile.update_wizard_status('resource_management',
        ...
        quota_inquiries = quota_inquiries.reject do |quota_inquiry|
          quota_inquiry.aasm_state == 'closed'
        end
        ...
        if quota_inquiries.length.positive?
          ...
          status = if approved_inquiries.length.positive?
                     ProjectProfile::STATUS_DONE
                   else
                     ProjectProfile::STATUS_PENDING
                   end
          ...
          @project_profile.update_wizard_status(
            ...
            aasm_state: inquiry.aasm_state,
            package: inquiry.payload['package']
          )
        else
    ...
    def update_cost_control_wizard_status
      billing_data = service_user.domain_admin_service(:cost_control)
    ...
    def update_networking_wizard_status
      if current_user.has_role?('admin') && !current_user.has_role?('network_admin')
        network_admin_role = services.identity
                                     .grant_project_user_role_by_role_name(
                                       @scoped_project_id,
                                       current_user.id,
                                       'network_admin'
                                     )
        # HACK: extend current_user context to add the new assigned role
        current_user.context['roles'] << { 'id' => network_admin_role.id,
